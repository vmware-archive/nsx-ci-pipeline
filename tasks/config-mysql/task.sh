...
MYSQL_GUID=$(./om-cli/om-linux -t https://$OPS_MGR_HOST -k -u $OPS_MGR_USR -p $OPS_MGR_PWD \
...
for job_guid in $(./om-cli/om-linux -t https://$OPS_MGR_HOST -k -u $OPS_MGR_USR -p $OPS_MGR_PWD \
                              curl -p "/api/v0/staged/products/${MYSQL_GUID}/jobs" 2>/dev/null \
                              | jq '.[] | .[] | .guid' | tr -d '"')
do
      ...
      if [ "$?" == "0" ]; then
        ...
        do
          ...
          ./om-cli/om-linux -t https://$OPS_MGR_HOST -k -u $OPS_MGR_USR -p $OPS_MGR_PWD \
              curl -p "/api/v0/staged/products/${MYSQL_GUID}/jobs/${job_guid}/resource_config" \ 
                -X PUT \ 
              -d "${NSX_LBR_PAYLOAD}"
